...
struct MultiLabelLeidenInput : public function::CallFuncInputBase {
  ~MultiLabelLeidenInput() = default;
  MultiLabelLeidenInput() = default;
  ...
  bool parse_subgraph(const ::physical::Subgraph& subgraph,
    ...
    if (parsed.vertex_entries.empty()) {
      LOG(ERROR) << "multi_label_leiden requires at least one vertex label.";
      return false;
    }
    if (parsed.edge_entries.empty()) {
      LOG(ERROR) << "multi_label_leiden requires at least one edge label.";
      return false;
    }
    // Check no predicates (not supported yet)
    for (const auto& ve : parsed.vertex_entries) {
      if (ve.predicate != nullptr) {
        LOG(ERROR)
            << "Vertex predicates are not supported in multi_label_leiden.";
        return false;
      }
    }
    for (const auto& ee : parsed.edge_entries) {
      if (ee.predicate != nullptr) {
        LOG(ERROR)
            << "Edge predicates are not supported in multi_label_leiden.";
        return false;
      }
    }
    // Extract all vertex labels
    for (const auto& ve : parsed.vertex_entries) {
      vertex_labels.push_back(ve.label);
    }
    // Extract all edge triplets
    for (const auto& ee : parsed.edge_entries) {
...
std::unique_ptr<function::CallFuncInputBase> MultiLabelLeidenFunction::bind(
  ...
  auto input = std::make_unique<MultiLabelLeidenInput>();
  if (!input->parse_subgraph(subgraph, ctx_meta)) {
    LOG(ERROR) << "Failed to parse subgraph for multi_label_leiden.";
    THROW_NOT_SUPPORTED_EXCEPTION("Invalid subgraph for multi_label_leiden");
  }
...
execution::Context MultiLabelLeidenFunction::exec(
    const function::CallFuncInputBase& input_base, neug::IStorageInterface& g) {
  const auto& input =
      dynamic_cast<const MultiLabelLeidenInput&>(input_base);
...
function::function_set MultiLabelLeidenFunction::getFunctionSet() {
