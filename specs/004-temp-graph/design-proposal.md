...
| 加载临时边表   | `LOAD EDGE TABLE FROM <file> (<options>) AS <label>`         |
...
-- 加载临时边表（临时点 → 持久化点）
LOAD EDGE TABLE FROM 'purchases.csv' (
...
LOAD EDGE TABLE FROM '/data/purchases.csv' (
...
[Parser]
  ...
  transformLoadNodeTable() → 生成 LoadAs Statement（is_edge=false, where=age>18, return=[user_id,name]）
  │
  ▼
[Binder] bindLoadAs()
...
**新增**：`LOAD NODE TABLE` / `LOAD EDGE TABLE` 语法规则与 `LoadAs` Statement 类。

- `Cypher.g4`：新增 `nEUG_LoadNodeTable` 和 `nEUG_LoadEdgeTable` 规则，包含可选的 `oC_Where` 和 `RETURN` 子句
- `load_as.h`（新增）：`LoadAs` Statement 类，包含 source、target_label、is_edge、primary_key、from/to/from_col/to_col、wherePredicate（可选）、returnColumns（可选）等字段
- Transformer：新增 `transformLoadNodeTable()` / `transformLoadEdgeTable()`，文件源解析复用现有 `transformScanSource()`，WHERE 解析复用现有 `oC_Where` 规则
