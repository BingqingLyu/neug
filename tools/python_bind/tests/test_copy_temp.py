...
class TestCopyTemp:
    ...
    def setup(self, tmp_path):
        ...
        self.people_csv = _write_csv(
            self.csv_dir, "people.csv",
            "id|name|age\n1|Alice|30\n2|Bob|25\n3|Carol|35\n4|Dave|20\n")
        self.edges_csv = _write_csv(
            self.csv_dir, "edges.csv",
            "src_id|dst_id|weight\n1|2|0.5\n2|3|1.0\n3|4|0.8\n")
        self.edges_shuffled_csv = _write_csv(
            self.csv_dir, "edges_shuffled.csv",
    ...
    def _load_persistent_person_table(self):
        ...
        for vid, name, age in [(1, "Alice", 30), (2, "Bob", 25), (3, "Carol", 35), (4, "Dave", 20)]:
            self.conn.execute(f"CREATE (p:Person {{id: {vid}, name: '{name}', age: {age}}});")
    ...
    def test_copy_temp_node_basic(self):
        self.conn.execute(f'COPY TEMP TempPeople FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        rows = list(self.conn.execute("MATCH (n:TempPeople) RETURN n.id, n.name ORDER BY n.id;"))
        assert len(rows) == 4 and rows[0][1] == "Alice"

    def test_copy_temp_node_default_pk(self):
        self.conn.execute(f'COPY TEMP TempDefault FROM "{self.people_csv}" (header = true)')
    ...
    def test_copy_temp_node_where(self):
        ...
        rows = list(self.conn.execute("MATCH (n:TempFiltered) RETURN n.name ORDER BY n.id;"))
    ...
    def test_copy_temp_rel_basic(self):
        ...
        self.conn.execute(f'COPY TEMP TempPerson FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        self.conn.execute(
    ...
    def test_copy_temp_rel_persistent(self):
        ...
        assert len(list(self.conn.execute("MATCH ()-[r:TempKnowsP]->() RETURN r;"))) == 3
    ...
    def test_copy_temp_rel_shuffled_needs_return(self):
        ...
        rows = list(self.conn.execute("MATCH (a:Person)-[r:TempShuf]->(b:Person) RETURN a.id, b.id ORDER BY a.id;"))
        assert len(rows) == 3 and rows[0] == [1, 2]
    ...
    def test_copy_temp_rel_return_no_extra_props(self):
        ...
        self.conn.execute(
            ...
            f"(from = 'Person', to = 'Person')")
        rows = list(self.conn.execute("MATCH (a:Person)-[r:TempNoProps]->(b:Person) RETURN a.id, b.id ORDER BY a.id;"))
        assert len(rows) == 3 and rows[0] == [1, 2]
    ...
    def test_copy_temp_rel_missing_from_to(self):
        ...
        rows = list(self.conn.execute("MATCH (n:TempEdge) RETURN n.src_id ORDER BY n.src_id;"))
        assert len(rows) == 3
    ...
    def test_copy_temp_cleanup(self):
        self.conn.execute(f'COPY TEMP TempE FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
    ...
    def test_duplicate_label_same_connection(self):
        ...
        self.conn.execute(f'COPY TEMP TempDup FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        with pytest.raises(Exception):
            self.conn.execute(f'COPY TEMP TempDup FROM "{self.people_csv}" (primary_key = \'id\', header = true)')

    def test_reload_same_label_after_close(self):
        """After close() cleanup, reloading same label succeeds."""
        self.conn.execute(f'COPY TEMP TempReuse FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        assert list(self.conn.execute("MATCH (n:TempReuse) RETURN count(n);"))[0][0] == 4
        self.conn.close()
        conn2 = self.db.connect()
        try:
            conn2.execute(f'COPY TEMP TempReuse FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
            assert list(conn2.execute("MATCH (n:TempReuse) RETURN count(n);"))[0][0] == 4
        finally:
            conn2.close()

    def test_temp_not_persisted_after_db_reopen(self):
        """Temp labels not in checkpoint; absent after DB reopen."""
        self.conn.execute(f'COPY TEMP TempGhost FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        assert list(self.conn.execute("MATCH (n:TempGhost) RETURN count(n);"))[0][0] == 4
        self.conn.close()
        self.db.close()
    ...
    def test_persistent_survives_temp_cleanup(self):
        ...
        self.conn.execute(f'COPY TEMP TempGone FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        self.conn.close()
        conn2 = self.db.connect()
        try:
            assert list(conn2.execute("MATCH (n:Persistent) RETURN count(n);"))[0][0] == 1
            with pytest.raises(Exception):
    ...
    def test_temp_and_persistent_join(self):
        ...
        self._load_persistent_person_table()
        extra_csv = _write_csv(self.csv_dir, "extra.csv", "id|nickname\n1|Ally\n2|Bobby\n5|Eve\n")
        self.conn.execute(f'COPY TEMP TempExtra FROM "{extra_csv}" (primary_key = \'id\', header = true)')
        rows = list(self.conn.execute(
    ...
    def test_temp_src_persistent_dst(self):
        ...
        self._load_persistent_person_table()
        self.conn.execute(f'COPY TEMP TempSrc FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        self.conn.execute(
    ...
    def test_empty_csv(self):
        ...
        self.conn.execute(f'COPY TEMP TempEmpty FROM "{empty_csv}" (primary_key = \'id\', header = true)')
        assert list(self.conn.execute("MATCH (n:TempEmpty) RETURN count(n);"))[0][0] == 0
    ...
    def test_property_type_inference(self):
        ...
        self.conn.execute(f'COPY TEMP TempTyped FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        rows = list(self.conn.execute("MATCH (n:TempTyped) RETURN n.age + 1 ORDER BY n.id;"))
        assert rows[0][0] == 31  # Alice age=30

    def test_multiple_queries_on_same_temp(self):
        """Multiple queries on same temp table all work."""
        self.conn.execute(f'COPY TEMP TempStable FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        assert list(self.conn.execute("MATCH (n:TempStable) RETURN count(n);"))[0][0] == 4
        rows = list(self.conn.execute("MATCH (n:TempStable) WHERE n.age > 25 RETURN n.id ORDER BY n.id;"))
        assert len(rows) == 2
        assert list(self.conn.execute("MATCH (n:TempStable) RETURN sum(n.age);"))[0][0] == 110

    def test_aggregation_on_temp(self):
        """SUM, MIN, MAX on temp table."""
        self.conn.execute(f'COPY TEMP TempAgg FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        rows = list(self.conn.execute("MATCH (n:TempAgg) RETURN sum(n.age), min(n.age), max(n.age);"))
    ...
    def test_label_conflict_with_persistent(self):
        ...
        with pytest.raises(Exception):
            self.conn.execute(f'COPY TEMP Conflict FROM "{self.people_csv}" (primary_key = \'id\', header = true)')

    def test_dangling_reference_silent_skip(self):
        """Edge with non-existent vertex key is silently skipped."""
        dangling_csv = _write_csv(self.csv_dir, "dangling.csv",
            "src_id|dst_id|weight\n1|2|0.5\n99|3|1.0\n")
        self.conn.execute(f'COPY TEMP TempPD FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        self.conn.execute(
    ...
    def test_rel_with_where(self):
        ...
        self.conn.execute(f'COPY TEMP TempPW FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        self.conn.execute(
    ...
    def test_rel_where_and_return(self):
        ...
        self.conn.execute(f'COPY TEMP TempPWR FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        self.conn.execute(
    ...
    def test_drop_temp_node(self):
        ...
        self.conn.execute(f'COPY TEMP TempDrop FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
    ...
    def test_drop_temp_edge(self):
        ...
        self.conn.execute(f'COPY TEMP TempNodeK FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        self.conn.execute(
        ...
        assert list(self.conn.execute("MATCH (n:TempNodeK) RETURN count(n);"))[0][0] == 4

    def test_drop_then_recreate(self):
        """After DROP, same label can be reused."""
        self.conn.execute(f'COPY TEMP TempRecycle FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        self.conn.execute("DROP TABLE TempRecycle;")
        self.conn.execute(f'COPY TEMP TempRecycle FROM "{self.people_csv}" (primary_key = \'id\', header = true)')
        assert list(self.conn.execute("MATCH (n:TempRecycle) RETURN count(n);"))[0][0] == 4
...
class TestCopyTempJsonl:
    ...
    def test_node_from_jsonl(self):
        ...
        rows = list(self.conn.execute("MATCH (n:TempJP) RETURN n.fName ORDER BY n.fName;"))
    ...
    def test_node_from_jsonl_with_where(self):
        ...
        rows = list(self.conn.execute("MATCH (n:TempJOld) RETURN n.fName, n.age ORDER BY n.age;"))
        assert len(rows) >= 3
        for row in rows:
            assert row[1] >= 35
    ...
    def test_node_from_jsonl_with_return(self):
        ...
        rows = list(self.conn.execute("MATCH (n:TempJSlim) RETURN n.fName ORDER BY n.fName;"))
        assert len(rows) == 8
        with pytest.raises(Exception):
...
class TestCopyTempJson:
    ...
    def test_node_from_json(self):
        ...
        rows = list(self.conn.execute("MATCH (n:TempJArr) RETURN n.fName ORDER BY n.fName;"))
        assert len(rows) == 8 and rows[0][0] == "Alice"
    ...
    def test_node_from_json_with_where(self):
        self.conn.execute(
            ...
            f"(primary_key = 'ID') WHERE age >= 35 RETURN *)")
        rows = list(self.conn.execute("MATCH (n:TempJArrOld) RETURN n.age ORDER BY n.age;"))
...
class TestCopyTempParquet:
    ...
    def setup(self, tmp_path):
        ...
        if not os.path.exists(self.parquet_path):
            pytest.skip(f"Parquet file not found")
    ...
    def test_node_from_parquet(self):
        ...
        rows = list(self.conn.execute("MATCH (n:TempPq) RETURN n.fName ORDER BY n.fName;"))
        assert len(rows) == 8 and rows[0][0] == "Alice"
    ...
    def test_node_from_parquet_with_where(self):
        ...
        rows = list(self.conn.execute("MATCH (n:TempPqOld) RETURN n.age ORDER BY n.age;"))
...
class TestCopyTempRemoteHttpfs:
    ...
    def test_node_from_remote(self):
        ...
        rows = list(self.conn.execute("MATCH (n:TempRemote) RETURN n.fName ORDER BY n.fName;"))
        assert len(rows) == 8 and rows[0][0] == "Alice"
    ...
    def test_node_from_remote_with_where(self):
        self.conn.execute(
            ...
            f"(primary_key = 'ID') WHERE age >= 40 RETURN *)")
        rows = list(self.conn.execute("MATCH (n:TempRemoteOld) RETURN n.age ORDER BY n.age;"))
        assert all(row[0] >= 40 for row in rows)
...
class TestCopyTempReadOnlyRejection:
    ...
    def setup(self, tmp_path):
        ...
        os.makedirs(self.csv_dir, exist_ok=True)
        self.people_csv = _write_csv(self.csv_dir, "people.csv",
            "id|name|age\n1|Alice|30\n2|Bob|25\n")
